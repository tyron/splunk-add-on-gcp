...
class BaseRestHandler(admin.MConfigHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (self.requestedAction == admin.ACTION_CREATE and
                self.callerArgs.id and
                self.callerArgs.id.startswith('_')):
            RH_Err.ctl(400,
    ...
    def check_caps(self):
        current_caps = user_caps(rest.makeSplunkdUri(),
                                 self.getSessionKey())
    ...
    def decode(self, name, ent):
        ...
        # decrypt
        # Workaround the ridiculous param
        if True or self.callerArgs.data.get('--get-clear-credential--') == ['1']:
            try:
...
class BaseModel(object):
    ...
    """
    # REST prefix. Change it if needed.
    rest_prefix = ''
    ...
    def normalize(self, data):
        ...
        for k, vs in data.items():
            ...
            data[k] = [self.normalisers[k].normalize(v, data) for v in vs] \
                ...
                else self.normalisers[k].normalize(vs, data)
        return data
    ...
    def validate(self, data):
        ...
        for f, vs in data.items():
            ...
            for v in vs:
                if not self.validators[f].validate(v, data):
                    RH_Err.ctl(
                        1100,
                        msgx=('field="{f}" - {msg}'
                              ''.format(msg=self.validators[f].msg, f=f)),
