...
class DispatchEngine(object):
    ...
    def _make_url(self, host, port, endpoint,
                  resource=None, do_sync=False):
        """
        ...
        if host and port:
            url = "https://{}:{}/servicesNS/nobody/{}/{}".format(host,
                                                                 port,
                                                                 app_name,
                                                                 endpoint)
        else:
    ...
    def _handle_global_settings(self):
        ...
        for global_setting in global_settings_unchanged:
            ...
            with self._forwarders_lock.reader_lock:
                available_forwarders = {forwarder_load.forwarder for
                                        forwarder_load in
                                        self._available_forwarderloads}
        ...
        for global_setting in global_settings_changed:
            with self._forwarders_lock.reader_lock:
                forwarders_to_deploy = {forwarder_load.forwarder for
                                        forwarder_load in
                                        self._available_forwarderloads}
    ...
    def _sort_global_settings(self, global_settings):
        ...
        tmp = [(global_setting,
                ordered_global_setting_schemas.index(
                    DispatchSnapshotManager.split_name(global_setting)[0])) for
               global_setting in global_settings]
    ...
    def _deploy_global_setting(self, forwarder, global_setting):
        ...
        if response is None or response.status not in (200, 201):
            ...
            log.logger.error("message=\"Deploy global setting:%s to "
                             "forwarder:%s error\" "
                             "detail_info=\"%s\"",
                             global_setting, forwarder, content)
            return self.DISPATCH_FAIL
    ...
    def _update_input(self, _input):
        ...
        if response is None or response.status not in (200, 201):
            ...
            log.logger.error("message=\"Update input setting:%s "
                             "to forwarder:%s error\" "
                             "detail_info=\"%s\"", _input, forwarder, content)
            return self.DISPATCH_FAIL

        # Update snapshot
        self._dispatch_snapshot_manager.update_input(_input,
                                                     input_setting,
                                                     None)
        try:
            self._dispatch_snapshot_manager.sync()
    ...
    def _delete_input(self, _input):
        ...
        try:
            response, content = sr.splunkd_request(url, session_key,
                                                   method="DELETE")
        except Exception:
            self._update_unavailable_forwarders(forwarder,
                                                self.DEPLOY_INPUT)
    ...
    def _update_unavailable_forwarders(self, forwarder, action):
        ...
        with self._forwarders_lock.reader_lock:
            if forwarder in {forwarder_load.forwarder for
                             forwarder_load in
                             self._available_forwarderloads}:
